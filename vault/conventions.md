...
- **Exit codes.** `2` parse failure; `1` fetch failure or unexpected throw; otherwise pass through the script's exit code. `runInstall` returns the code; `main.ts` assigns it to `process.exitCode` rather than calling `process.exit()` so in-process tests can `await main()`.
